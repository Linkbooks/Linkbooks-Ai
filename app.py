...
def login():
    if request.method == 'GET':
        # Pass chatSessionId to the login template if present in the query parameters
    ...
    try:
        ...
        chat_session_id = data.get('chatSessionId')  # Retrieve chatSessionId from the hidden input
...
def quickbooks_login():
    ...
    try:
        ...
        # Decode the session token to extract the user ID
        try:
...
def start_oauth_for_chatgpt():
    ...
    try:
        ...
        if not user_check.data:
            ...
            logging.info(f"ChatGPT session not linked to a user. Redirecting to login: {middleware_login_url}")
...
def link_chat_session():
    ...
    try:
        ...
        logging.info(f"Payload for chatgpt_oauth_states upsert: {payload}")

        # Perform upsert with conflict resolution on user_id
        response = supabase.table("chatgpt_oauth_states").upsert(payload, on_conflict=["user_id"]).execute()

        if not response.data:
            logging.error(f"Failed to link chatSessionId {chat_session_id} for user {user_id}. Response: {response}")
...
def get_session_status():
    ...
    """
    try:
        # Retrieve chatSessionId from query parameters
        chat_session_id = request.args.get('chatSessionId')
        if not chat_session_id:
            return jsonify({"authenticated": False, "message": "chatSessionId is required"}), 400

        # Query Supabase for the session
...
def fetch_reports_route():
    """
    Fetches QuickBooks reports for a given ChatGPT session.
    """
    try:
        ...
        # Fetch tokens for the given ChatGPT session
        response = supabase.table("chatgpt_tokens").select("*").eq("chat_session_id", chat_session_id).execute()
        if not response.data or len(response.data) == 0:
        ...
        if expiry and datetime.utcnow() > datetime.fromisoformat(expiry):
            ...
            try:
                ...
                realm_id = refreshed_tokens.get('realm_id', realm_id)
            except Exception as e:
        ...
        report_data = fetch_report(
            user_id=realm_id,  # Use realm_id as the user identifier
...
def business_info():
    """
    Retrieves the user's company information from QuickBooks.
    """
    try:
        ...
        if not access_token or not realm_id:
            logging.error("QuickBooks not connected for user.")
